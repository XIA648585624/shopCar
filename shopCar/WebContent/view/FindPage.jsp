...
<html>
  ...
  <jsp:include page="Menu.jsp"></jsp:include>
   ...
   <!--  按条件搜索表单 -->
   <center>
   <form action="${path }/view/FindProductPageAction" method="post">
   <select name="pageSize">
   页面大小<option value="5">5</option>
  ...
  <select name="pageSize">
   ...
   <c:if test="${currentPage!=1}">
     <a href="${path }/view/FindProductPageAction?currentPage=${currentPage-1}&pageSize=${sessionScope.pageSize}">上一页</a>
    </c:if>
    &nbsp;
    这是第 <font color="red">${sessionScope.currentPage }/${pageNum }</font> 页
    &nbsp;
   <c:if test="${currentPage!=pageNum}">
    <a href="${path }/view/FindProductPageAction?currentPage=${currentPage+1}&pageSize=${sessionScope.pageSize}">下一页</a>
    &nbsp;
   </c:if> 
</center>
